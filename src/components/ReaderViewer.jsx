...
export default function ReaderViewer({ chapter, manga, prevChapter, nextChapter, mangaSlug }) {
  ...
  const [scrollSpeed, setScrollSpeed] = useState(1); // Default 1.0
  const scrollInterval = useRef(null);

  // 1. Initial Load (Cek Notice)
  useEffect(() => {
  ...
  useEffect(() => {
    if (isAutoScrolling) {
      ...
      // Buat interval baru berdasarkan speed
      // Semakin besar speed, interval semakin kecil (lebih cepat update)
      // Atau pixel scroll semakin besar. Kita pakai pixel scroll statis, interval dinamis agar smooth.
      const step = 1; // 1px per tick
      const baseDelay = 15; // ms
      // Logic: speed 1.0 = 15ms. speed 1.5 = 10ms (lebih cepat). speed 0.5 = 30ms (lebih lambat).
  ...
  const toggleAutoScroll = () => {
    ...
    // Jika mulai scroll, sembunyikan UI agar enak bacanya
  ...
  return (
    <div className="relative min-h-screen bg-[#111] text-white font-sans selection:bg-primary/30">
      ...
      >
        ...
        {chapter.images && chapter.images.map((imgUrl, index) => (
          <img 
            ...
            src={imgUrl} 
            alt={`Page ${index + 1}`} 
            className="w-full h-auto max-w-3xl shadow-lg block mb-0.5" // mb-0.5 biar tidak ada gap putih
            loading="lazy" 
          />
        ))}
      ...
      <div className={`fixed bottom-6 left-0 w-full z-40 px-4 transition-transform duration-300 ease-in-out ${showUI ? 'translate-y-0' : 'translate-y-[200%]'}`}>
         <div className="max-w-[350px] mx-auto bg-[#222]/95 backdrop-blur-xl border border-gray-700/50 rounded-full px-6 py-2 shadow-2xl flex items-center justify-between">
            ...
            <button 
                ...
                className="flex flex-col items-center gap-1 text-gray-400 hover:text-white transition group"
            >
                <div className="p-2 rounded-full group-hover:bg-gray-700/50">
                    <Settings size={20} />
                </div>
      ...
      {/* === 4. MODAL SETTINGS (Sesuai Gambar Capture.PNG) === */}
      {showSettings && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200" onClick={() => setShowSettings(false)}>
            <div 
                className="bg-[#111] w-full max-w-sm rounded-xl p-6 shadow-2xl border border-gray-800 relative animate-in zoom-in-95 duration-200"
            ...
            >
                ...
                <div className="mb-8">
                    ...
                    <p className="text-center text-primary text-xs font-bold mt-2">
                        Current: {scrollSpeed}x
                    </p>
                </div>
                ...
                <div className="flex items-center gap-4">
                    <button 
                        ...
                        className="flex-1 bg-[#8b5cf6] hover:bg-[#7c3aed] text-white font-bold py-3 rounded-lg text-sm transition shadow-[0_0_15px_rgba(139,92,246,0.3)]"
                    >
                        Save
                    </button>
                    <button 
                        onClick={() => setShowSettings(false)}
                        className="flex-1 bg-transparent hover:bg-gray-800 text-white font-medium py-3 rounded-lg text-sm transition"
                    >
                        Later
                    </button>
                </div>
      ...
      {showNotice && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4 animate-in fade-in duration-200">
          <div className="bg-[#1e1e1e] w-full max-w-sm rounded-2xl p-6 shadow-2xl border border-gray-800 relative animate-in zoom-in-95 duration-200">
            <h3 className="text-white font-bold text-center mb-4 text-base leading-relaxed">
                Tekan Gambar Komik Untuk Mode Full Screen
            </h3>
            ...
            >
                {dontShowAgain ? (
                    ...
                    <Square className="text-gray-500 group-hover:text-gray-400" size={20} />
                )}
                <span className="text-sm text-gray-300 select-none group-hover:text-white">Jangan Tampilkan Lagi</span>
            </div>
            <button 
                onClick={handleCloseNotice}
                className="w-full py-2.5 bg-[#8b5cf6] hover:bg-[#7c3aed] text-white font-bold rounded-lg transition shadow-lg shadow-purple-900/20 active:scale-95"
            >
                Tutup
            </button>
          </div>
