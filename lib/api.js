...
export async function getMangaList(params = {}) {
  const { q, genre, type, status, page = 1, limit = 24 } = params;

  let endpoint = '';

  if (q && q.trim()) {
    endpoint = `/search?q=${encodeURIComponent(q.trim())}&page=${page}&limit=${limit}`;
  } else if (genre && genre !== 'all') {
    const g = encodeURIComponent(genre).replace(/%20/g, '-');
    endpoint = `/filter/genre/${g}?page=${page}&limit=${limit}`;
  } else if (type && type !== 'all') {
    endpoint = `/filter/type/${encodeURIComponent(type)}?page=${page}&limit=${limit}`;
  } else if (status && status !== 'all') {
    endpoint = `/filter/status/${encodeURIComponent(status)}?page=${page}&limit=${limit}`;
  } else {
    endpoint = `/manga-list?page=${page}&limit=${limit}`;
  }
